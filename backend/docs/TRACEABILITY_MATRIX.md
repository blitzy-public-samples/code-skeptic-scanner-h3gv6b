...
identifier has a complete row in the log — `DL-001` … `DL-292`, one unbroken sequence of 292 rows — and no
...
two files in `backend/docs`, for **84** delivered artifacts. Section 2 carries one row for each of them,
...
**1599** cases, all passing. The class-by-class case counts are maintained in §2.7 and nowhere else.
...
| `backend/docs/DECISION_LOG.md` | *No source construct — net-new* — required by Rule 1; DL-058 | 296 entries, `DL-001` … `DL-296`, with no gap and no repeat, each carrying decision, alternatives, rationale and risks. Every identifier cited anywhere in this repository resolves to one of them |
...
| `config/DataSourceConfig.java` | `db/database.py:L5-13` — the per-call `create_engine` and `sessionmaker` | One pooled `DataSource` built from the translated URL (A14). Pool geometry and timing are the framework defaults; no `scanner.datasource.pool` key and no `spring.datasource` key is read (DL-027, DL-270) |
...
| `repository/TweetRepository.java` | `db/database.py:L10-13` — the per-call session | `JpaRepository<Tweet, Integer>` (DL-138) declaring six members: the keyset batch `findUnansweredBatchAfter(Integer, Pageable)` expressing `t.responses is empty` (A8, DL-248), `findByIdForUpdate`, the `AnalysisSubject` projection read, the `updateDoubtRating` write, the `TweetAggregate` summary read and the `DailyTrend` window read. A page of tweets is read through the inherited `findAll(Pageable)` (DL-249) |
| `repository/ResponseRepository.java` | `db/database.py:L10-13` — the per-call session | `JpaRepository<Response, Integer>` declaring the `ResponseRow` projection page read with an explicit count query (DL-245, DL-249), the `ApprovalCounts` summary read, `existsByTweetId` and `findByIdForUpdate` with its lock-wait hints |
...
| `api/TweetControllerTest.java` | `tests/test_api.py` | 54 | All three routes, the 1 and 10 defaults, the 404 literal, and the non-numeric path identifier answering 404 (DL-048) |
| `api/ResponseControllerTest.java` | `tests/test_api.py` | 111 | All four routes and every status code and literal, including the two-outcome contract of DL-076 |
...
| `service/SentimentAnalysisServiceTest.java` | `tests/test_services.py:L58-65` | 60 | Doubt-rating boundaries at scores −1, 1, 0, −0.5 and 0.5, the out-of-range clamp at −2 and 2, and the `NaN` parity value of 10.0 (DL-036) |
| `service/TwitterServiceTest.java` | `tests/test_services.py:L12-22` | 138 | Replaces the two `pass` stubs. Popularity gate at 99, 100 and 101 against the default of 100, and the settings-row precedence of DL-255 |
| `service/NotionServiceTest.java` | `tests/test_services.py:L28-38` | 126 | The delivered POST query, POST create and PATCH update; the 2,000-character item split and its concatenating inverse at eight lengths from 1 to 12,345 (DL-292); the retry classification of DL-253 |
| `service/LlmServiceTest.java` | `tests/test_services.py:L44-48` | 172 | Asserts against the delivered Chat Completions call; the source's `generate_text` does not exist |
| `service/ResponseServiceTest.java` | *No source construct — net-new* — DL-076, DL-211 | 139 | The four route-facing operations, the two background entry points, and the parent-row lock guard of DL-195 |
| `service/SettingsServiceTest.java` | *No source construct — net-new* — DL-043 | 65 | The two route-facing operations, the three-row seed of DL-040 and the insert-only path of DL-159 |
...
| `task/ResponseGenerationSchedulerTest.java` | `tests/test_tasks.py` | 27 | The `@Scheduled` fixed-delay declaration, the per-pass candidate ceiling of DL-282 and the ownership gate of DL-250 |
| `task/TweetStreamListenerTest.java` | `tests/test_tasks.py` | 45 | Required-member validation (DL-223), the persist path closing D1, and the two-level mirror reporting of DL-224 |
| `repository/JpaMappingIntegrationTest.java` | `db/models.py` | 50 | `@DataJpaTest`: the four table names, all twenty column names, the association ordering, the ten wide character columns and the bounded primary key, and the vendor-independent invariants that no generated character type is a large-object type and every generated primary-key column is indexable (DL-068, DL-069, DL-166) |
...
| `config/DatabaseUrlTranslatorTest.java` | *No source construct — net-new* — DL-027, DL-064, DL-071, DL-072, DL-187 | 119 | All five accepted schemes, the `+driver` suffix strip, the `jdbc:` pass-through, credential extraction, and the fail-fast on an unrecognised scheme and on an empty value |
...
| Delivered test classes | 19 | 19 rows, 1599 cases | §2.7 |
