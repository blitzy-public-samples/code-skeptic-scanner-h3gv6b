...
Agent Action Plan names is present: sixty files under `backend/src/main/java`, which is the fifty-eight
of AAP 0.3.1 plus `api/ErrorDispatchController` (DL-183) and `util/LogSafe` (DL-119). Six test classes
named by the plan are outside this checkpoint's processing boundary and are not present on disk —
`api/TweetControllerTest`, `api/ResponseControllerTest`, `api/AnalyticsControllerTest`,
`ScannerApplicationTests`, `task/ResponseGenerationSchedulerTest` and `task/TweetStreamListenerTest` —
and fifteen are delivered, the thirteen planned ones that have landed plus the two additional classes
DL-096 authorises. Every source construct a pending target would carry is still listed below, with its
...
| 16 | `backend/app/tasks/tweet_monitoring.py` | `task/TweetStreamClient` and `task/TweetStreamListener` (DL-044/DL-045/DL-046), with `config/WebClientConfig`, the `WebClient` bean replacing the tweepy `Stream` construction at `:L45-51` | Delivered |
...
| 20 | `backend/tests/test_tasks.py` | `task/ResponseGenerationSchedulerTest` (7 cases) and `task/TweetStreamListenerTest` (19), both delivered, together with the two classes covering `task/TweetStreamClient` (DL-214). The import at `:L3` names neither a module nor symbols that exist, so nothing carries forward from it; the rate-limiting wish at `:L55` is deliberately not honoured | Delivered |
...
| 10 | `GET /analytics/trends` — 200 | `analytics.py:L7-15 get_trends` | `api/AnalyticsController.getTrends` | `service/AnalyticsService.getTrends()` (DL-042) | delivered (controller); service Delivered |
| 11 | `GET /analytics/summary` — 200 | `analytics.py:L17-25 get_summary` | `api/AnalyticsController.getSummary` | `service/AnalyticsService.getSummary()` (DL-041) | delivered (controller); service Delivered |
...
| 18 | `settings` | `key String primary_key` (`:L42`) | `entity/Setting.key` `String` | `@Id @Column(name = "\"key\"")` — quoted reserved word (DL-061), no declared length, generated as `varchar(255)` on all three supported vendors (DL-069) | Delivered |
...
`scanner.ingestion.stream-base-keywords` (DL-044), and the stream activation, timeout, backoff and
bounded-dispatch settings (DL-198). `server.port` is `${PORT:5000}` (DL-029) and
...
#### 1.5.1 Seeded `settings` rows and their consumers

`service/SettingsService` seeds three rows idempotently on `ApplicationReadyEvent` (DL-040, DL-159) and
`PUT /settings/{key}` edits them. Every one has a runtime consumer, and each consumer applies the same
precedence: the row overrides the configured property, and an absent, `null` or unusable stored value
falls back to that property with one `WARN` naming the key and never the stored value.
...
| 8 | celery | `tasks/response_generation.py:L1` | `@EnableScheduling` plus a completion-based `TriggerTask` — no broker (DL-047/DL-194) | Retired |
...
| A9 | `Tweet.get(tweet_id)` and `response.save()` are called on declarative models, which expose neither | `tasks/response_generation.py:L16,L25-26` | `repository/TweetRepository.findById` and `repository/ResponseRepository.save`, called from `service/ResponseService`; the scheduler that also needs them is | Delivered |
...
| A15 | The client sends camelCase member names and an `/api` prefix that the backend never served, imports an `authService` module that does not exist, and calls a `generate-response` endpoint that does not exist | `frontend/src/schema/*.ts`, `frontend/src/services/api.ts:L25,L43`, `frontend/src/utils/api.ts:L13-16` | Not reconciled; the backend retains snake_case and unprefixed routes (DL-022/DL-059) | Retired |
...
| 7 | HUMAN ASSISTANCE NEEDED | `services/llm_service.py:L34` | `service/LlmService.generateResponse` returns the generated text as a `String` (DL-177) instead of the source's partial dictionary, and `service/ResponseService` builds the wire record from the stored row | Delivered |
...
| 16 | TODO: Implement response generation logic | `tasks/tweet_monitoring.py:L32` | `task/TweetStreamListener` calling `service/ResponseService.generateResponse`, which is delivered | Delivered |
...
Three files, 183 lines, none of which can import. The Agent Action Plan names nineteen JUnit classes as
their replacement; thirteen of those are delivered, alongside the two additional classes DL-096
authorises, for fifteen delivered test classes carrying 733 cases in total.

| Source test file | Source defect | JUnit replacements | Status |
|------------------|---------------|--------------------|--------|
| `backend/tests/test_api.py` | Imports `fastapi.testclient.TestClient` at `:L2` and points it at a Flask application; `test_get_settings` at `:L37-39` requests `/settings/` with a trailing slash and expects a key-to-value map; `test_update_settings` at `:L41-44` `PUT`s to `/settings/`, which is not a registered route; `:L50-51` names `total_tweets` and `total_responses`, which is the one piece of usable evidence in the file | Delivered: `api/GlobalExceptionHandlerTest` (47 cases over both error envelopes and every translated exception type, DL-181), `api/SettingControllerTest` (33), `api/AuthControllerTest` (82) and `api/ErrorDispatchControllerTest` (60, DL-096/DL-183). Also delivered: `api/TweetControllerTest` (27), `api/ResponseControllerTest` (26), `api/AnalyticsControllerTest` (11) and `ScannerApplicationTests` (36) — DL-216. The two settings expectations are deliberately discounted (DL-039) and the summary key names are honoured (DL-041) | Partly delivered |
| `backend/tests/test_services.py` | Wrong package root at `:L3-6` (`from services.…`); two `pass` stubs at `:L12-22`; tests for `create_page`, `update_page` and `generate_text` at `:L28-38,L44-48`, none of which exist; asserts sentiment analysis returns `'positive'`/`'negative'`/`'neutral'` at `:L58-65` where the implementation returns a float | Delivered: `service/SentimentAnalysisServiceTest` (48 cases), `service/NotionServiceTest` (47), `service/LlmServiceTest` (70), `service/SettingsServiceTest` (56), `service/SettingsServiceSeedingIntegrationTest` (11), `service/TwitterServiceTest` (60), `service/ResponseServiceTest` (40, including the single-owner guard of DL-196), `service/AnalyticsServiceTest` (33). The two `pass` stubs at `:L12-22` are replaced by the popularity-gate matrix of `service/TwitterServiceTest` | Delivered |
| `backend/tests/test_tasks.py` | `from backend.tasks import monitor_tweets, generate_response` at `:L3` — neither the module path nor either symbol exists | Delivered: `task/ResponseGenerationSchedulerTest` (7 cases) and `task/TweetStreamListenerTest` (19) | Delivered |

Five delivered test classes descend from none of the three retired test files: the Python suite tested
none of what they cover. Three of the five have no source construct of any kind and are net-new:
...
`api/AuthControllerTest` (82 cases, DL-019 — also counted against `test_api.py` above, since it tests a
...
`security/JwtServiceTest` (66 cases) covers `core/security.py:L6-12`, and
`repository/JpaMappingIntegrationTest` (38 cases) covers `db/models.py`.

`backend/tests/test_tasks.py` remains the one retired test file with no delivered replacement: both
test classes named against it, `task/ResponseGenerationSchedulerTest` and `task/TweetStreamListenerTest`,
are listed in §3. The two production classes they would cover are themselves delivered.
...
| `backend/docs/DECISION_LOG.md` | *No source construct — net-new* — required by Rule 1 | Two hundred and eighteen entries, `DL-001` … `DL-218`, each carrying decision, alternatives, rationale and risks. Every identifier cited anywhere in this repository resolves to one of them |
...
| `config/AsyncSchedulingConfig.java` | `main.py:L41-48` and `tasks/response_generation.py:L8` — the blocking initialiser and the broker-less Celery application | `@EnableScheduling` plus the single `taskScheduler` bean, whose only configured value is the thread-name prefix (DL-208); no broker and no queue (DL-047) |
...
| `security/JwtAuthenticationFilter.java` | The `@jwt_required` decorator sites — a guard that enforced nothing | A `OncePerRequestFilter` that actually validates the bearer token (A1, DL-021); it writes one fixed sentence on acceptance and nothing at all on refusal, carrying no request method, path, token or principal (DL-144/DL-203) |
...
| `api/ResponseController.java` | `api/responses.py:L8-65` — all four routes | Paths, methods and every status code and literal preserved: 400 `Tweet ID is required`, 201, 500 `Failed to generate response`, 400 `Update data is required`, 404 `Response not found or update failed` (DL-076). The route path never consults the background generation guard (DL-196) |
...
| `api/ErrorDispatchController.java` | *No source construct — net-new* — DL-182. The behaviour it reproduces is `main.py:L31-37`: the retired tree registered no `/error` route, so a request for that path fell to `@app.errorhandler(404)` | Answers the servlet `ERROR` dispatch, which `sendError` puts beyond the reach of `api/GlobalExceptionHandler`. Withdraws Spring Boot's `BasicErrorController` by implementing the `ErrorController` marker; emits only the six literals the advice emits; copies no path, status, timestamp or exception detail into a body; restates the six Spring Security response headers the `ERROR` dispatch skips |
...
| `dto/ResponseDto.java` | `schema/response.py:L4-9` | Five components; `id` and `tweet_id` serialise as strings; non-null `id` invariant (DL-177) with the four other components nullable (DL-080/DL-139) |
...
| `dto/UpdateResponseRequest.java` | `api/responses.py:L54` — the free-form `request.json` | Partial update of `content` and/or `is_approved` only, held as raw JSON nodes so a key's presence is independent of its value (DL-082) |
...
| `dto/SummaryDto.java` | `api/analytics.py:L23-25`, with two key names evidenced at `tests/test_api.py:L50-51` | Seven aggregate metrics read under one snapshot (DL-041/DL-075/DL-091/DL-180) |
...
| Target file | Source construct | Notes |
|-------------|------------------|-------|
| `entity/Tweet.java` | `db/models.py:L8-18` plus the relationship attached at `:L30` | `@Table(name = "tweets")`, nine columns, `@OneToMany` with `@OrderBy("id ASC")` and the lazy default (DL-162); `Integer` identifier (DL-070/DL-138); identifier-based `equals`/`hashCode` (DL-203) |
| `entity/Response.java` | `db/models.py:L21-28` | `@Table(name = "responses")`, five columns, `@ManyToOne(fetch = LAZY) @JoinColumn(name = "tweet_id")` (DL-025/DL-162); identifier-based `equals`/`hashCode` (DL-203) |
| `entity/AiTool.java` | `db/models.py:L32-37` | `@Table(name = "ai_tools")`, three columns, no association (DL-070); identifier-based `equals`/`hashCode` (DL-203) |
| `entity/Setting.java` | `db/models.py:L39-44` | `@Table(name = "settings")`, `key` as `@Id`; both reserved-word columns quoted (DL-061/DL-069); identifier-based `equals`/`hashCode` (DL-203) |
...
| `repository/ResponseRepository.java` | `db/database.py:L10-13` | `JpaRepository<Response, Integer>` with `countByIsApprovedTrue()` for the `approved_responses` metric (DL-041) and `existsByTweetId(Integer)` for the generation existence guard (DL-196); the page read carries an entity graph (DL-087) |
...
| `service/mapper/TweetMapper.java` | *Net-new class, derived from* the `to_dict()` called at `api/tweets.py:L19,L30` and never defined | Identifier-to-string and delimited-column-to-array conversion (A7, DL-023/DL-024); an explicit hand-written mapper bean (DL-204) that carries a `null` identifier through (DL-205) |
| `service/mapper/ResponseMapper.java` | *Net-new class, derived from* the `to_dict()` called at `api/responses.py:L18,L29,L47,L63` and never defined | Same, for `responses` (A7, DL-204); rejects an entity carrying no identifier (DL-205) |
...
| `service/LlmService.java` | `services/llm_service.py:L16-32` | Chat Completions, the three call literals preserved, the generated text returned as a `String` (DL-032 … DL-035, DL-177, DL-178) |
| `service/ResponseService.java` | *Net-new class, derived from* the four call sites at `api/responses.py:L15,L26,L44,L60`; the class was imported at `:L3` and never existed (D4) | Signatures dictated by the call sites; never publishes to X (DL-076/DL-178). One injected singleton holding five collaborators (DL-211); the background-only `generateResponseIfAbsent` is the single generation owner (DL-196). Pagination normalisation is DL-200, `generated_at` minting DL-201 and identifier parsing DL-202. A fifth operation, `generateResponseIfAbsent(String)`, is the single entry point of both background paths and carries the claim plus the transaction-scoped existence guard (DL-196); the route path is unguarded and unchanged |
...
| `task/TweetStreamListener.java` | `tasks/tweet_monitoring.py:L8-34` — the `TweetListener` class that subclassed nothing, so `on_status` was never invoked | `onStatus(JsonNode)` returning `true` to continue streaming; the four documented steps in order; the entity built with the real column names in place of the `text`/`user`/`sentiment` arguments at `:L22-28`; persistence closing the `TODO` at `:L29`; the generation trigger closing the `TODO` at `:L32`; the generated reply mirrored to Notion, which closes the call to the absent `update_tweet_response` at `tasks/response_generation.py:L30` on the ingestion path; string-contract fields require a JSON string and counts require an integral number; a rejected record is recorded by category and correlation token, never by payload (DL-049/DL-052/DL-194/DL-197/DL-199) |
...
delivered suite rather than estimated; the thirty-one rows below sum to the 1381 cases
...
|-------------|------------------|-------|-------|
| `ScannerApplicationTests.java` | `tests/test_api.py` — the import the file could not perform | 36 | The context-load smoke test plus the composition root of DL-209: one application context, the two annotations `ScannerApplication` declares, and the absence of a second application object |
| `api/AnalyticsControllerTest.java` | `api/analytics.py:L7-25` and `tests/test_api.py:L47-59` | 11 | Both zero-argument routes through `@WebMvcTest`: the summary metric set, the day-bucketed trends envelope, and that neither route accepts a query parameter (DL-041/DL-042) |
| `api/AuthControllerTest.java` | *No source construct — net-new* — DL-019 | 90 | The token route end to end through `@WebMvcTest` with the real `SecurityConfig` imported: issuance without an `Authorization` header, the credential-length ceiling, the empty 401 for every rejection, the bearer challenge, scheme casing, the body-size limit, and the bcrypt configuration matrix including the unresolved-placeholder guard (DL-116/DL-118/DL-189) |
| `api/ErrorDispatchControllerTest.java` | *No source construct — net-new* — DL-096/DL-183 | 63 | The container's `ERROR` dispatch through the single `ErrorController` on `${server.error.path}`: both envelopes, the status carried from the request attribute, and a header the `REQUEST` dispatch already wrote keeping exactly one value (DL-194) |
| `api/GlobalExceptionHandlerTest.java` | `main.py:L31-37` and `tests/test_api.py` | 90 | Both error envelopes byte-for-byte, every per-route literal, and the three-way split of the `HttpMessageConversionException` hierarchy driven through the framework's own converter and resolver (DL-092/DL-188) |
| `api/ResponseControllerTest.java` | `api/responses.py:L8-65` and `tests/test_api.py` | 26 | All four routes with every status code and literal, `page` and `per_page` bound as text with the 1 and 10 fallbacks, and an unparseable identifier answering 404 rather than 400 (DL-048/DL-217) |
...
| `api/TweetControllerTest.java` | `api/tweets.py:L9-55` and `tests/test_api.py` | 27 | The three routes, the pagination envelope, degenerate `page` values normalised to the first page, the `Tweet not found` literal and the analyze result shape (DL-037/DL-048/DL-217) |
...
| `config/DatabaseUrlTranslatorTest.java` | *No source construct — net-new* — DL-027/DL-064/DL-071/DL-072 | 47 | Translation, credential extraction and rejection, six look-alike properties, ports, schemes, redaction |
| `config/RestClientConfigTest.java` | `services/notion_service.py:L8` — `Client(auth=…)` | 20 | The Notion base URL, the version header actually sent from configuration and the bearer token, plus the CR/LF guard on a configured header value (DL-013) |
...
| `service/LlmServiceTest.java` | `tests/test_services.py:L44-48` | 130 | The delivered Chat Completions call, the generation-failure matrix and lazy-client behaviour (DL-167/DL-168) |
| `service/NotionServiceTest.java` | `tests/test_services.py:L28-38` | 92 | All three operations plus transport failures, unconfigured database id, null bodies and cursor handling |
| `service/ResponseServiceTest.java` | *No source construct — net-new* | 69 | The four operations the call sites at `api/responses.py:L15,L26,L44,L60` dictate, and the absence of any publish path |
| `service/SentimentAnalysisServiceTest.java` | `tests/test_services.py:L58-65` | 48 | Finite and clamping vectors, NaN and both infinities, null input, propagated client failure and client lifecycle |
...
| `service/TwitterServiceTest.java` | `services/twitter_service.py:L42-50` and `tests/test_services.py:L12-22` | 83 | The popularity gate: the inclusive boundary at 99/100/101 against the configured default, the same boundary at two other thresholds, the `settings` row overriding configuration, the configured fallback for a row holding no integer, whitespace discarded, a threshold at or below zero honoured as stored, and an absent like count answered without reading the table (DL-040) |
| `task/ResponseGenerationSchedulerTest.java` | `tests/test_tasks.py` and `tasks/response_generation.py:L35-50` | 7 | One pass over the candidates `findByResponsesIsEmpty()` returns, per-candidate isolation so one failure does not end the pass, the skip when a reply appeared after the query ran, and the pass summary (DL-195) |
...
| `task/TweetStreamClientTest.java` | `tasks/tweet_monitoring.py:L36-55` and `services/twitter_service.py:L16-23` | 70 | The protocol through a stubbed `ExchangeFunction`: app-only token exchange, rule reconciliation, NDJSON reassembly across chunk boundaries, `429` with `x-rate-limit-reset`, backoff with jitter, reconnect on a clean end and the blank-credential stop (DL-207/DL-214) |
| `task/TweetStreamListenerTest.java` | `tests/test_tasks.py` and `tasks/tweet_monitoring.py:L8-34` | 19 | The four ingestion steps in order, persistence closing the `TODO` at `:L29`, the generation trigger closing the `TODO` at `:L32`, the type-checked payload reads and the UTC substitution for an unparseable `created_at` (DL-195/DL-199) |
...
| `util/QueryParametersTest.java` | `api/tweets.py:L12-13` and `api/responses.py:L11-12` | 25 | The conversion matrix of DL-217: absent, blank, whitespace, non-numeric, signed, overflowing and non-ASCII digit input against the declared defaults |
...
in §2.7: `api/TweetControllerTest` (27 cases), `api/ResponseControllerTest` (26),
`api/AnalyticsControllerTest` (11), `ScannerApplicationTests` (36),
`task/ResponseGenerationSchedulerTest` (7) and `task/TweetStreamListenerTest` (19). Their arrival closes
...
carry a committed test of their own rather than being covered only indirectly. The suite is thirty-one
classes running 1381 cases; the additions beyond the Agent Action Plan's inventory are enumerated and
...
|--------------|----------|------------------|-----------|---------|
| Retired Python files (§1.1) | 20 | 20 | 18 fully, 2 partly (test files only) | 0 |
...
| Additional defects A1–A15 (§1.8) | 15 | 15 | 14 fully, 1 retired by decision | 0 |
| Scaffolding markers (§1.9) | 23 | 23 | 23 | 0 |
| Python test files (§1.10) | 3 | 3 | 1 fully, 2 partly | 0 |
| Delivered main Java classes (§2.2–§2.6) | 62 | 62 | 62 | — |
| Delivered test Java classes (§2.7) | 31 | 31 | 31 | — |
