...
identifier has a complete row in the log — `DL-001` … `DL-285`, one unbroken sequence (285 rows) — and no
...
was read back from the retired files at commit `80f1d53d^`. The delivered tree holds **sixty-nine**
files under `backend/src/main/java` and **forty-seven** under `backend/src/test/java` — one hundred and
sixteen Java classes — together with `backend/pom.xml`, `backend/.gitignore`, `backend/.dockerignore`,
`backend/src/main/resources/application.yml`, `backend/src/test/resources/application-test.yml` and the
two files in `backend/docs`, for **one hundred and twenty-three** delivered artifacts. Section 2 carries one
...
twenty-eight test classes beyond the frozen test inventory of nineteen are admitted by DL-096 and
enumerated by DL-216, so what is present is forty-seven files under `backend/src/test/java`, which is
the plan's nineteen plus the twenty-eight that inventory could not name. Twenty-four of the forty-seven
delivered test classes carry a row naming a source construct, and the remaining twenty-three have no source
...
and the stream activation, timeout, backoff and bounded-dispatch settings (DL-198). Twenty-six further
...
(DL-281), the per-pass candidate ceiling (DL-282) and the four provider budget and circuit
keys (DL-283);
...
`src/main/resources/application.yml` declares **fifty-five** environment placeholders and **nine** further
...
| 52 | `PROVIDER_CALLS_PER_WINDOW` | `scanner.background.provider-calls-per-window` | `500` | No | DL-283 |
| 53 | `PROVIDER_WINDOW_SECONDS` | `scanner.background.provider-window-seconds` | `3600` | No | DL-283 |
| 54 | `PROVIDER_FAILURE_THRESHOLD` | `scanner.background.provider-failure-threshold` | `5` | No | DL-283 |
| 55 | `PROVIDER_CIRCUIT_OPEN_SECONDS` | `scanner.background.provider-circuit-open-seconds` | `300` | No | DL-283 |
...
Thirty-four of the forty-seven delivered test classes descend from none of the three retired test files:
the Python suite tested none of what they cover. Twenty-three of the forty-seven have no source construct of any
...
(DL-040), `task/BackgroundOwnershipTest` (DL-281/DL-284), `task/ProviderWorkBudgetTest`
(DL-282/DL-283), `util/ConfiguredValuesTest` (DL-197), `util/LogSafeTest` (DL-208),
...
| `backend/docs/DECISION_LOG.md` | *No source construct — net-new* — required by Rule 1 | Two hundred and eighty-five entries, `DL-001` … `DL-285`, with no gap and no repeat, each carrying decision, alternatives, rationale and risks. Every identifier cited anywhere in this repository resolves to one of them |
...
| `task/ProviderWorkBudget.java` | *No source construct — net-new* — DL-283. The retired tree bounded neither the volume of external calls nor the repetition of a failing one | The fixed-window allowance and consecutive-failure circuit both provider paths consult: `task/ResponseGenerationScheduler` per candidate before generating and mirroring, and `task/TweetStreamListener` immediately ahead of the sentiment call (DL-283). State is one process's — two counters and an instant under `synchronized`, read against the one clock bean of DL-278 — so it needs no store and adds no schema. Records are emitted on transition only, so a refusing budget writes no record per refused unit (DL-052). A refusal skips the work; nothing is queued and nothing is published to X |

### 2.7 Tests

The `Cases` column records the number of tests Surefire executes for the class, measured on the
delivered suite rather than estimated; the forty-seven rows below sum to the 2492 cases
...
| `config/AsyncSchedulingConfigTest.java` | `tasks/response_generation.py:L41-50` — the `while True` loop | 37 | The completion-based `TriggerTask`: the next instant is the previous completion plus the effective delay, so pacing is fixed **delay** and not fixed rate; the `settings` row overrides configuration and the pool is one thread (DL-047/DL-195) |
...
| `service/ResponseServiceTest.java` | *No source construct — net-new* | 136 | The four route operations plus the background-only generation operation, canonical identifier claims, parent-lock orchestration, writable-value updates including the ten carried values the record refuses and the explicit `null` it admits, real mapper coverage, the absence of any publish path, and the finite page-size bound of DL-123 (DL-081/DL-082/DL-122/DL-123/DL-195/DL-231/DL-244) |
...
| `task/ProviderWorkBudgetTest.java` | *No source construct — net-new* — DL-283 | 27 | The two bounds against a hand-advanced clock: the allowance exhausted and rolled over at the window boundary, the circuit opened at the configured consecutive-failure count and closed again after its interval, a success clearing the count, `remainingInWindow()` across both states, and that a record is written on transition only |
...
| `.github/workflows/ci.yml` | `L16-19`, `L24`, `L26-29`, `L35-39`, `L47-50`, `L56-59` | `setup-python` 3.9, `node-version: '14'`, `pip install -r backend/requirements.txt`, `flake8`, `mypy`, `pytest`, `python -m build` | JDK 21 (Temurin) with Maven caching in place of the Python toolchain step, and the single command `mvn -B clean verify` with `working-directory: ./backend` in place of the four retired Python steps (DL-004/DL-005/DL-057). Two values carry further changes: the toolchain action inside `L16-19` is `actions/setup-java` pinned to a 40-hex commit SHA on the `node24` runtime line with its resolved release beside it (DL-103), and the `node-version` input at `L24` is the mandatory Node.js floor `22.23.2` (DL-285). Every other line is byte-identical to the source branch: the single `build-and-test` job, `actions/checkout@v2`, `actions/setup-node@v2`, `npm ci`, the frontend lint and type-check step whose scripts the manifest does not declare, `npm test`, `npm run build` and the staging placeholder; no `permissions` key, no `persist-credentials` input and no job split (DL-059/DL-074/DL-104/DL-215) |
...
forty-seven classes running 2492 cases, measured from the delivered Surefire run; `api/ErrorDispatchControllerTest`
...
| Delivered main Java classes (§2.2–§2.6) | 69 | 69 | 69 | — |
| Delivered test Java classes (§2.7) | 47 | 47 | 47 | — |
...
| Delivered test cases (§2.7) | — | 47 classes | 2492 cases, 0 failing | — |
| **Delivered artifacts under `backend/`** | **123** | **123** | **123** | **0** |
