 ...
 *
 * <p>The set of client-visible messages is closed: exactly the four literals declared as constants
 * here, reached from five source branches, each carried by its own factory —
 * {@link #tweetNotFound()} ({@code backend/app/api/tweets.py:L32} and {@code :L43}),
 * {@link #responseNotFound()} ({@code backend/app/api/responses.py:L31}),
 * {@link #responseNotFoundOrUpdateFailed()} ({@code :L65}) and {@link #settingNotFound()}
 * ({@code backend/app/api/settings.py:L22}). The two response-scoped literals stay separate messages at
 * the same status, the shorter reporting the read branch and the longer the update branch. An
 * unparseable path variable is reported through this type as well — DL-048.
 *
